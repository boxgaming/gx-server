...
Import GXSUI From "https://boxgaming.github.io/gx-server/lib/ui/ui.bas"
...
GXSUI.Init GXSUI.DOCK_RIGHT, GXSUI.CHAT_DISABLED, GXSUI.RESIZE_DISABLED
InitPlayers
StartGame
...
Sub StartGame
    ...
    While Not connecting
       ...
       ElseIf opt = "J" or "JOIN" Then
           GXSUI.ShowGameListDialog "Sample Game"
           connecting = GX_TRUE
       End If
    Wend
End Sub
...
Sub OnStartHostGame (playerName As String, gameDesc As String)
    ...
    pname = playerName
    GXS.HostGame "Sample Game", gameDesc
End Sub
...
Sub OnHostGame (msg As Object)
    GXSUI.PutMessage "Game started with id: " + GXS.SessionId 
    connecting = GX_FALSE
...
Sub OnJoinGame (msg As Object)
   If GXS.IsHost Then
        GXSUI.PutMessage "New player joined with client id: " + msg.cid 
        AddPlayer msg.cid
    Else
        connecting = GX_FALSE
        GXSUI.PutMessage "Joined game with client id: " + msg.cid
...
Sub OnError (msg As Object)
    If connecting Then
        GXSUI.PutMessage "An error occurred while attempting to connect to the game server."
        connecting = GX_FALSE
        StartGame
    Else
        GXSUI.PutMessage "An unknown error occurred."
    End If 
End Sub

' Called when any client disconnects from the server
Sub OnClientDisconnected (msg As Object)
    GXSUI.PutMessage "Client disconnected: " + msg.cid
    Dim p As Player
    p = pmap(msg.cid)
    p.active = GX_FALSE
End SUb
...
Sub OnHostDisconnected (msg As Object)
    GXSUI.PutMessage "Host has disconnected"
    GXSceneStop
    System
End Sub
...
Sub OnUpdate (e As GXEvent)
    If GXKeyDown(GXKEY_ESC) Then 
        GXS.LeaveGame
        GXSUI.PutMessage "You've left the game"
        System
    End If
